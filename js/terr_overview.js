...
function openTerrOverview() {
  ...
  _loadTerrData().then(() => _renderTable('')).catch(e => {
    console.warn('terr_overview: Daten nicht geladen:', e);
    const tbody = document.getElementById('terr-tbody');
    if (tbody) tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:#7a5a3a;padding:20px">⚠</td></tr>';
  });
  // Focus search
  setTimeout(() => {
...
async function _loadTerrData() {
  ...
  _terrData = await fetchLocalizedJson('data/knowledge/territories.json');
  const rulerRes = await fetch('data/knowledge/rulers.json', { cache: 'no-cache' });
  if (!rulerRes.ok) throw new Error('rulers.json HTTP ' + rulerRes.status);
