...
// Removed: tooltip.js already wires makeDraggable(tt, hdr) onto the pinned
// tooltip. This second drag system stacked its own mousemove delta on top,
// so dragging the header moved the tooltip at double mouse speed.
...
async function init() {
  ...
  window.addEventListener('resize', () => {
    ...
    // Events layer keeps its own projection-dependent positions
    if (typeof eventsMode !== 'undefined' && eventsMode > 0 &&
        typeof renderEvents === 'function') {
      renderEvents(pctToYear(state.pct));
    }
  });
