...
function _showTooltipEl() {
  ...
  // Default: links visible. showEventClusterTT hides them again after this
  // call — without the reset its display:none leaked into every later tooltip.
...
function createCityPanel(mouseEvent, city) {
  ...
  panel.querySelector('.cp-wiki').addEventListener('click', () =>
    window.open('https://en.wikipedia.org/wiki/' + wikiSlug, '_blank', 'noopener'));
  if (city.wikidata) {
    ...
    wdEl.addEventListener('click', () =>
      window.open('https://www.wikidata.org/wiki/' + city.wikidata, '_blank', 'noopener'));
  }
  ...
  if (_summary) {
    ...
    if (modern) {
      desc.innerHTML =
        ...
        `ⓘ ${t('tt_modern_site')}: <span style="color:#9a8a62">${escHtml(modern)}</span></div>` +
        `<div>${escHtml(txt)}</div>`;
    } else {
...
function createTerritoryPanel(event, props, color) {
  ...
  const wdHtml = mapping ? `<div class="tt-wiki tp-wiki-wd">
    ...
    <span class="tp-wd-txt">Wikidata ${escHtml(mapping.wikidataId)} →</span></div>` : '';

  panel.innerHTML = `
    <div class="tp-handle">
      <img class="tp-flag" alt="">
      <span class="tp-title-group"><span class="tp-type-icon">▣</span><span class="tp-name">${escHtml(name)}</span></span>
      <span class="tp-btn tp-snap" title="Snap to top" onclick="snapPanel('${id}')">⊟</span>
  ...
  panel.querySelector('.tp-wiki-wp').onclick = () =>
    window.open('https://en.wikipedia.org/wiki/' + wikiName, '_blank', 'noopener');
  if (mapping) {
    panel.querySelector('.tp-wiki-wd').onclick = () =>
      window.open('https://www.wikidata.org/wiki/' + mapping.wikidataId, '_blank', 'noopener');
  }
  ...
  if (mapping) {
    ...
    loadTerritoriesData().then(territories => {
      ...
      if (capital) {
        ...
        panel.querySelector('.tp-capital').innerHTML = `<a href="${wikiUrl}" target="_blank" class="tt-capital-link">${escHtml(capital)}</a>`;
...
function renderRulersInPanel(knowledge, currentYear, panel) {
  ...
  if (knowledge.error) {
    rulerArea.innerHTML = `<div class="tt-loading" style="color:#5a3a2a">${escHtml(knowledge.error)}</div>`;
    return;
  }
  ...
  rulerArea.innerHTML = knowledge.rulers.map((r, i) => {
    ...
    return `<div class="tt-ruler tt-ruler-link" onclick="openRulerDetail(document.getElementById('${pid}')._rulers[${i}], document.getElementById('${pid}'))">${escHtml(r.name)}<span class="tt-ruler-years">(${s}–${e})</span></div>`;
  }).join('');
}
...
function showTooltip(event, props, color) {
  ...
  // ── Mobile / narrow viewport: no hover tooltip. The click path opens the
  //    bottom sheet (_mSheetOpenTerritory); the old tooltip-bar path this
  //    branch fed (showMobileTooltip) was removed in the mobile overhaul and
  //    threw a ReferenceError on narrow desktop windows.
  if (isMobile()) return;
  ...
  wpEl.onclick = () => window.open(`https://en.wikipedia.org/wiki/${wikiName}`, '_blank', 'noopener');
  ...
  if (mapping) {
    ...
    wdEl.onclick = () => window.open(`https://www.wikidata.org/wiki/${mapping.wikidataId}`, '_blank', 'noopener');
  } else {
  ...
  if (mapping) {
    ...
    loadTerritoriesData().then(territories => {
      ...
      if (capEl && capRow) {
        if (capital) {
          const wikiUrl = `https://en.wikipedia.org/wiki/${encodeURIComponent(capital.replace(/ /g,'_'))}`;
          capEl.innerHTML = `<a href="${wikiUrl}" target="_blank" class="tt-capital-link">${escHtml(capital)}</a>`;
...
function renderRulers(knowledge, currentYear) {
  ...
  rulerArea.innerHTML = knowledge.rulers.map((r, i) => {
    const s = r.start ? fmt(r.start) : '?';
    const e = r.end   ? (r._endInferred ? 'c. '+fmt(r.end) : fmt(r.end)) : '–';
    return `<div class="tt-ruler tt-ruler-link" onclick="openRulerDetail(window._currentRulers[${i}])">${escHtml(r.name)}<span class="tt-ruler-years">(${s}–${e})</span></div>`;
  }).join('');
}
...
function showCityTT(event, city) {
  ...
  const descHtml = _cityDescTxt
    ? `<li style="padding-left:0;color:#b0a070">${escHtml(_cityDescTxt)}</li>` : '';
  ...
  document.getElementById('tt-wiki-txt').textContent = t('tt_wiki_arrow');
  document.getElementById('tt-wiki-wp').onclick = () => window.open('https://en.wikipedia.org/wiki/'+wikiName,'_blank','noopener');
  const wdEl = document.getElementById('tt-wiki-wd');
  if (city.wikidata) {
    wdEl.style.display = 'flex';
    document.getElementById('tt-wd-txt').textContent = 'Wikidata '+city.wikidata+' →';
    wdEl.onclick = () => window.open('https://www.wikidata.org/wiki/'+city.wikidata,'_blank','noopener');
...
function openRulerDetail(ruler, sourcePanel) {
  ...
  const imgHtml = ruler.image
    ? `<img src="${ruler.image}" class="rp-img" style="opacity:0;transition:opacity 200ms ease-out" onload="this.style.opacity=1" alt="${escHtml(ruler.name)}">`
    : '';
  ...
  panel.innerHTML = `
    <div class="rp-drag-handle">
      ...
      <span class="rp-title-group"><span class="rp-type-icon">♛</span><span class="rp-title">${escHtml(territoryLabel || ruler.name)}</span></span>
    ...
    <div class="rp-body">
      ${imgHtml}
      <div class="rp-ruler-name">${escHtml(ruler.name)}</div>
      <div class="rp-years">${t('tt_ruled')} ${s} – ${e}</div>
      <div class="rp-links">
        <a href="${wikiUrl}" target="_blank" class="rp-link">${t('tt_wiki_arrow')}</a>
        ${wdUrl ? `<a href="${wdUrl}" target="_blank" class="rp-link">Wikidata ${escHtml(ruler.wikidataId)} →</a>` : ''}
      </div>
    </div>
  ...
  // Self-cleaning: deregisters itself once the panel is gone, otherwise one
  // document-level listener accumulated per opened ruler panel.
  const _onPanelTouchEnd = () => {
    pStartX = undefined;
    if (!panel.isConnected) document.removeEventListener('touchend', _onPanelTouchEnd);
  };
  document.addEventListener('touchend', _onPanelTouchEnd);
...
// ── Tooltip Modus-Button (Größe) ─────────────────────────────
// (Der 'move'-Modus samt setupTooltipMove war toter Code — der Button
//  dafür wurde nie eingebaut; Verschieben läuft über makeDraggable.)
let _ttMode = null; // 'resize' | null
...
// Cleanup for the document-level listeners attached by setupTooltipResize.
// Without this, document listeners accumulate on every mode toggle.
...
function _mSheetOpenTerritory(event, props, color) {
  ...
  const html = `
    ...
    <div class="ms-title" id="ms-name">${escHtml(name)}</div>
    ...
    <div class="ms-links">
      ...
      ${mapping ? `<a href="https://www.wikidata.org/wiki/${mapping.wikidataId}" target="_blank" class="ms-link">Wikidata ${escHtml(mapping.wikidataId)} →</a>` : ''}
    </div>`;
  ...
  if (mapping) {
    loadWikidataKnowledge(mapping, currentYear).then(knowledge => {
      ...
      if (!rulerArea) return;
      if (knowledge.error) {
        rulerArea.innerHTML = `<div class="ms-loading" style="color:#5a3a2a">${escHtml(knowledge.error)}</div>`;
      ...
      rulerArea.innerHTML = knowledge.rulers.map((r, i) => {
        const s = r.start ? fmt(r.start) : '?';
        const e = r.end   ? (r._endInferred ? 'c. '+fmt(r.end) : fmt(r.end)) : '–';
        return `<div class="ms-ruler" data-ridx="${i}">${escHtml(r.name)}<span class="ms-ruler-years">(${s}–${e})</span></div>`;
      }).join('');
    ...
    loadTerritoriesData().then(territories => {
      ...
      if (capital) {
        ...
        if (capEl && capRow) {
          const wikiUrl = 'https://en.wikipedia.org/wiki/' + encodeURIComponent(capital.replace(/ /g, '_'));
          capEl.innerHTML = `<a href="${wikiUrl}" target="_blank" class="ms-link">${escHtml(capital)}</a>`;
          capRow.style.display = 'flex';
        }
      }
...
function _mSheetOpenRuler(ruler, territoryLabel) {
  ...
  const wdUrl   = ruler.wikidataId ? 'https://www.wikidata.org/wiki/' + ruler.wikidataId : null;
  const imgHtml = ruler.image
    ? `<img src="${ruler.image}" class="ms-ruler-img" alt="${escHtml(ruler.name)}">`
    : '';
  ...
  const html = `
    ${imgHtml}
    <div class="ms-ruler-name">${escHtml(ruler.name)}</div>
    ${territoryLabel ? `<div class="ms-sub">${escHtml(territoryLabel)}</div>` : ''}
    <div class="ms-years-big">${s} – ${e}</div>
    <div class="ms-links">
      <a href="${wikiUrl}" target="_blank" class="ms-link">${t('tt_wiki_arrow')}</a>
      ${wdUrl ? `<a href="${wdUrl}" target="_blank" class="ms-link">Wikidata ${escHtml(ruler.wikidataId)} →</a>` : ''}
    </div>`;
...
function _mSheetOpenCity(city) {
  ...
  const _modernNote = _modern
    ? `<div style="font-size:10px;color:#7a6a4a;font-style:italic;margin-top:8px">ⓘ ${t('tt_modern_site')}: <span style="color:#9a8a62">${escHtml(_modern)}</span></div>`
    : '';

  const html = `
    <div class="ms-sub">${city.capital ? t('tt_capital_city') : t('tt_notable_city')}</div>
    ${_desc ? `<div class="ms-ctx-row" style="margin-top:6px">${escHtml(_desc)}</div>` : ''}
    ${_modernNote}
    ${_summaryTxt ? `<div class="ms-ctx-row" style="margin-top:6px;color:#a09060">${escHtml(_summaryTxt)}</div>` : ''}
    <div class="ms-links" style="margin-top:12px">
      <a href="https://en.wikipedia.org/wiki/${wikiName}" target="_blank" class="ms-link">${t('tt_wiki_arrow')}</a>
      ${wdUrl ? `<a href="${wdUrl}" target="_blank" class="ms-link">Wikidata ${escHtml(city.wikidata)} →</a>` : ''}
    </div>`;
...
function _mSheetOpenPleiades(city) {
  ...
  const html = `
    ...
    <div class="ms-links" style="margin-top:12px">
      <a href="https://en.wikipedia.org/wiki/${wikiName}" target="_blank" class="ms-link">${t('tt_wiki_arrow')}</a>
      ${wdUrl ? `<a href="${wdUrl}" target="_blank" class="ms-link">Wikidata ${escHtml(wdId)} →</a>` : ''}
    </div>`;
...
function _mSheetOpenEventCluster(evs) {
  ...
  const items = sorted.map((ev, i) => `
    <div class="ms-evcl-item" data-i="${i}">
      ...
      <span class="ms-evcl-title">${escHtml(ev.title)}</span>
    </div>`).join('');
...
function _mSheetOpenEvent(ev) {
  ...
  const imgHtml = ev.image
    ? `<img alt="${escHtml(ev.title)}" src="${ev.image}" style="width:100%;height:200px;object-fit:contain;background:rgba(0,0,0,0.55);border-radius:4px;margin-bottom:8px">`
    : '';
  const descHtml = ev.description
    ? `<div class="ms-ctx-row">${escHtml(ev.description)}</div>`
    : '';
