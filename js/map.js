...
//  RENDER
// ═══════════════════════════════════════════
let _renderEpochSerial = 0;

async function renderEpoch(idx) {
  const epoch = EPOCHS[idx];
  state.currentEpochIdx = idx;
  const serial = ++_renderEpochSerial;

  // Load while old epoch stays visible — no black gap
  const geo = await loadEpoch(epoch);
  if (!geo) return;
  // A newer renderEpoch started while we awaited the fetch — let it win,
  // otherwise the slower (uncached) epoch would overpaint the current one.
  if (serial !== _renderEpochSerial) return;
