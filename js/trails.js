...
async function loadTrailsData() {
  ...
  try {
    _trailsData = await fetchLocalizedJson('data/trails.json');
  } catch(e) {
    console.warn('trails.json not loaded:', e);
    _trailsData = [];
  }
  return _trailsData;
}
...
async function openTrailMenu() {
  ...
  menu.innerHTML =
    ...
    <div class="trail-menu-item" onclick="startTrail('${tr.id}')">
      <div class="trail-menu-title">${escHtml(tr.title)}</div>
      <div class="trail-menu-sub">${escHtml(tr.subtitle || '')} &middot; ${tr.steps.length} ${t('trail_steps')}</div>
    </div>`).join('');
...
function _renderTrailPanel(step, idx, total) {
  ...
  if (step.wiki) {
    ...
    wikiBtn.onclick = () => window.open('https://en.wikipedia.org/wiki/' + encodeURIComponent(step.wiki), '_blank', 'noopener');
  } else {
  ...
  if (step.wiki2) {
    ...
    wiki2Btn.onclick = () => window.open('https://en.wikipedia.org/wiki/' + encodeURIComponent(step.wiki2), '_blank', 'noopener');
  } else {
