...
// Zentraler Loader für sprachabhängige JSON-Daten.
// DE: versucht data/foo_de.json, fällt bei 404/Netzfehler auf data/foo.json
// zurück (vorher hatten events/tour/trails KEINEN EN-Fallback — fehlte die
// _de-Datei im Deploy, blieb der Layer leer).
async function fetchLocalizedJson(basePath, fetchOpts = { cache: 'no-cache' }) {
  const suffix = getDataSuffix();
  if (suffix) {
    const localized = basePath.replace(/\.json$/, suffix + '.json');
    try {
      const r = await fetch(localized, fetchOpts);
      if (r.ok) return await r.json();
      console.warn('fetchLocalizedJson: fallback auf EN für', localized);
    } catch (_) { /* fall through to EN */ }
  }
  const r = await fetch(basePath, fetchOpts);
