...
const _warnedSubjectos = new Set();

function getMappingForSubjecto(subjecto) {
  const key = normalize(subjecto);
  const mapping = TERRITORY_MAPPING[key] || null;
  // Nur einmal pro Key warnen — vorher feuerte das bei jedem Hover doppelt
  // (subj- und NAME-Lookup) und flutete die Konsole.
  if (!mapping && !_warnedSubjectos.has(key)) {
    _warnedSubjectos.add(key);
    console.warn('[EH] Unmapped SUBJECTO:', subjecto);
  }
  return mapping;
}
