...
function openSearch() {
  ...
  _srSelected = -1;
  // Preload data in background; re-run the query once the index is ready so a
  // user who typed before the load finished doesn't sit on stale "Loading…".
  _loadSrData().then(() => {
    const inp = document.getElementById('search-input');
    if (inp && inp.value.trim()) _doSearch(inp.value);
  }).catch(e => console.warn('search: Index nicht geladen:', e));
  setTimeout(() => {
    const inp = document.getElementById('search-input');
...
// (Lokalisiertes Laden läuft zentral über fetchLocalizedJson aus i18n.js.)
async function _loadSrData() {
  ...
  const [terrData, rulerData, citiesData, eventsRaw] = await Promise.all([
    fetchLocalizedJson('data/knowledge/territories.json'),
    fetch('data/knowledge/rulers.json')
      .then(r => { if (!r.ok) throw new Error('rulers.json HTTP ' + r.status); return r.json(); })
      .catch(e => { console.warn('search: rulers.json nicht geladen:', e); return {}; }),
    fetch('data/knowledge/cities.json').then(r => r.ok ? r.json() : null).catch(() => null),
    fetchLocalizedJson('data/events.json').catch(() => null),
  ]);
...
// Delegates to the central escaper (territory_render.js). Kept as a local
// alias so existing call sites stay unchanged.
function _escHtml(s) {
  return escHtml(s);
}
