...
// ── Central HTML escaper (defense-in-depth) ──
// Data strings from the Wikipedia crawls (ruler names, event titles, city
// descriptions, capital names, notes) are interpolated into innerHTML
// template literals all over the app. A stray `<` or `"` in a crawled name
// would break markup — contaminated data files would be XSS. Every module
// loaded after this file (map/tooltip/timeline/cities/events/app/intro/
// terr_overview/search/tour/trails) can rely on the global `escHtml`.
// Tolerant of null/undefined (returns ''); coerces non-strings.
function escHtml(s) {
  if (s == null) return '';
  return String(s)
    .replace(/&/g, '&amp;')
...
EH.territoryRender = (() => {
  ...
  function buildContextLines(ctx, variant = 'tooltip') {
    ...
    return fields
      .map(([key, i18nKey]) => ctx[key]
        ? `${wrapOpen}${labelTag(t(i18nKey))} ${escHtml(ctx[key])}${wrapClose}`
        : '')
      .filter(Boolean)
      .join('');
  }
  ...
  function buildEmptyRulerMessage(knowledge, variant = 'tooltip') {
    ...
    if (knowledge.note) {
      ...
      return `<div class="${wrapClass}" style="color:#9a8a62">${escHtml(noteText)}${more}</div>`;
    }
