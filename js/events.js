...
async function loadEventsData() {
  ...
  try {
    eventsData = await fetchLocalizedJson('data/events.json');
  } catch(e) {
...
function showEventTT(event, ev) {
  ...
  _currentEventYear = ev.year;  // keep "jump to year" in sync with this tooltip
  ...
  document.getElementById('tt-wiki-wp').onclick = () =>
    window.open('https://en.wikipedia.org/wiki/' + encodeURIComponent(ev.wiki), '_blank', 'noopener');
...
function showEventClusterTT(event, evs) {
  ...
  let html = top.map(e =>
    `<li style="padding-left:0">${_fmtEvYear(e.year)} · ${escHtml(e.title)}</li>`
  ).join('');
  if (sorted.length > 6) {
  ...
  moveTooltip(event);
  _showTooltipEl();
  // Hide wiki/links/goto in the cluster hover tooltip — must come after
  // _showTooltipEl(), which resets #tt-links to visible.
...
function _createEventClusterPanel(triggerEvent, evs) {
  ...
  const items = sorted.map((ev, i) => `
    <li class="evcl-item" data-i="${i}">
      ...
      <span class="evcl-title">${escHtml(ev.title)}</span>
    </li>`).join('');
